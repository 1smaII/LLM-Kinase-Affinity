...
def retry_oss(fn, *args, tries=5, **kwargs):
...
def oss_url(bucket, p, hours):
    ...
    if not retry_oss(bucket.object_exists, obj):
        mime = mimetypes.guess_type(p.name)[0] or "application/octet-stream"
        retry_oss(bucket.put_object_from_file, obj, str(p), headers={"Content-Type": mime})
...
class Qwen:
    ...
    def __init__(self, mode, cfg, max_tokens):
        self.mode, self.cfg, self.model, self.max_tokens = (
            mode,
            cfg,
            cfg["model"],
            max_tokens,
    ...
    def generate(self, prompt, text, images, xlsx):
        if self.mode == "text":
            ...
            for img in images:
                ...
                content.append(
                    {"type": "image_url","image_url": {"url": url},"max_pixels": self.cfg["max_pixels"]}
                )
            messages = [
                {
                    "role": "user",
        ...
        r = self.client.chat.completions.create(model=self.model,messages=messages,max_tokens=self.max_tokens,extra_body=extra)
...
class GPT:
    ...
    def generate(self, prompt, text, images, xlsx):
        ...
        r = self.client.responses.create(model=self.model,input=[{"role": "user", "content": content}],reasoning={"effort": self.cfg["reasoning_effort"]},max_output_tokens=self.max_tokens,background=True)
        print(f"GPT任务：{r.id}")
...
class Gemini:
    ...
    def _validation_groups(self, text):
        ...
        rows = data["sheets"][validation_i]["rows"]
        header = rows[0]
        validation_rows = rows[1:]
        outputs = []
    
        for i in range(0, len(validation_rows), self.validation_batch_size):
            chunk = json.loads(text)
            group = validation_rows[i:i + self.validation_batch_size]
            chunk["sheets"][validation_i]["rows"] = [header, *group]
            outputs.append(
                json.dumps(chunk,ensure_ascii=False,default=str,separators=(",", ":"))
            )
        return outputs
    ...
    def _run_one_batch(self, prompt, chunk_text, image_parts, index, total):
        ...
        if state != "JOB_STATE_SUCCEEDED":
            raise RuntimeError(
                f"Gemini Batch 未成功：{state}；错误：{getattr(job, 'error', None)}"
            )
